...
st.subheader("📊 Detailed Virality Table")

days_list = list(range(days))

table = pd.DataFrame({
    "Day": days_list,
    "Susceptible Viewers": np.array(S_list, dtype=int),
    "Active Sharers": np.array(I_list, dtype=int),
    "Dropped Viewers": np.array(R_list, dtype=int)
})
